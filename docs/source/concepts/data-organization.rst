...
Biological conditions and experimental metadata can be injected into the single-cell data by modifying the experiment configuration file. The items of the ``Labels`` section are structured to give unique information per well. In practice, whenever you have information that is different between at least two wells, use this section. The ``Metadata`` section, on the other hand, can be used to add some global variables to the single cell data (antibody concentration units, temperature and CO\ :math:`_2` conditions of the experiment, etc).