...
class DiskROIViewer(CellSizeViewer):
    """
    Tune the radius of a disk centred on the image, on a real frame.

    A :class:`CellSizeViewer` whose slider sets a radius, with the circle fixed at the exact image
    centre. The pixels outside the disk, which the computation ignores, are shaded.

    Parameters
    ----------
    parent_layout : QLayout
        Options layout whose ``radius_le`` field receives the radius on "Set".
    ...
    *args, **kwargs
    ...
    # Fraction of the disk covered by a cosine taper; 0 is a hard disk.
    alpha = 0.0
    ...
    def circle_center(self):
        """Exact image centre, the convention of :func:`radial_distance`."""
    ...
    def generate_weight_overlay(self):
        """Add the weight shading to the image axes."""

        # Distance map cached once, so a slider tick only re-evaluates the weights.
    ...
    def roi_weights(self) -> np.ndarray:
        """Weights of the pixels for the current radius."""
        return disk_taper(self.radial_distance, self.alpha, self.circle_radius())

    def update_circle(self):
        """Redraw the circle and the weight shading."""
        self.weight_rgba[..., 3] = 0.6 * (1.0 - self.roi_weights())
        self.im_weight.set_data(self.weight_rgba)
        super().update_circle()

    def set_threshold_in_parent_le(self):
        """Write the radius into the options layout and close."""
        self.parent_layout.radius_le.setText(f"{self.diameter_slider.value():.1f}")
        self.close()


class RegistrationROIViewer(DiskROIViewer):
    """
    Tune the correlation disk of the stack registration on a real frame.

    A :class:`DiskROIViewer` with the Tukey taper of
    :func:`celldetective.utils.registration.tukey_window`: a dashed circle at the start of the
    taper, a shading of the pixels by how little they weigh in the correlation (fully shaded
    pixels are ignored) and a Tukey α slider.
    ...
    *args, **kwargs
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        super().__init__(parent_layout, *args, **kwargs)
    ...
    def update_circle(self):
        ...
        self.circ_taper.set_radius(self.circle_radius() * (1.0 - self.alpha))
    ...
    def set_threshold_in_parent_le(self):
        ...
        super().set_threshold_in_parent_le()
