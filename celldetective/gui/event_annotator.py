...
class EventAnnotator(BaseAnnotator):
    ...
    def select_single_cell(self, index, timepoint):
        ...
        self.logger.info(f"You selected cell #{self.track_of_interest}...")
        self.give_cell_information()
        self.plot_signals()
    ...
    def save_trajectories(self):
        ...
        self.logger.info("Table successfully exported...")
        if self.class_choice_cb.currentText() != "":
            self.compute_status_and_colors(0)
        self.extract_scatter_from_trajectories()
...
class MeasureAnnotator(BaseAnnotator):
    ...
    def export_measurements(self):
        ...
        if pathsave != "":
            ...
            try:
                ...
                self.logger.info(f"File successfully written in {pathsave}.")
            except Exception as e:
    ...
    def compute_status_and_colors(self):
        ...
        if self.status_name not in self.df_tracks.columns:
            self.logger.info("Creating a new status for visualization...")
            self.make_status_column()
        else:
            self.logger.info(
                f'Generating per-state colors for the status "{self.status_name}"...'
            )
            all_states = self.df_tracks.loc[:, self.status_name].tolist()
            all_states = np.array(all_states)
            self.state_color_map = color_from_state(all_states, recently_modified=False)
            self.logger.info(f'Color mapping for "{self.status_name}":')
    ...
    def update_frame(self):
        ...
        if "TRACK_ID" in list(self.df_tracks.columns):
            ...
            self.logger.info("ID in cols... change class of interest... ")
    ...
    def save_trajectories(self):
        self.logger.info(f"Saving trajectories !!")
        if self.normalized_signals:
            self.normalize_features_btn.click()
        if self.selection:
            self.cancel_selection()
    ...
    def reload_frame(self):
        ...
        # self.redo_histogram()
            ...
            self.logger.error("Frame could not be loaded...")
