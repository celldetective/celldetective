...
class TableUI(CelldetectiveMainWindow):
    ...
    def perform_query(self):
        """
        Open a dialog to filter the table using a query string.
        """
    ...
    def plot1d(self):
        ...
        self.plot1dWindow = FigureCanvas(self.fig, title="Plot")
        self.ax.clear()
        ...
        if self.hue_cb.currentText() != "--":
            ...
            try:
                # Optimize: calc unique len once
                n_hues = len(self.data[self.hue_variable].unique())
                colors = [cmap(i / n_hues) for i in range(n_hues)]
            except:
                colors = None

        legend = True

        # Define a helper to reduce repetition of x vs y checking
        def get_plot_args():
            # Returns (x_var, y_var) to use based on availability
            if self.x is not None:
                ...
                return {
                    "x": self.y,
                    "y": None,
                }  # Some plots take x as primary, others y.
                # Actually seaborn is flexible. If x is None, we plot 'x' data on 'x' arg usually?
                # Original code logic:
                # if x is present -> use x=self.x, y=None (for hist) or y=self.y (for scatter)
                # if x is None and y is present -> use x=self.y (for hist)
            return {}

        # Seperate Logic for Univariate vs Bivariate plots slightly if needed,
        # but mostly seaborn handles it.
        # Replicating original "if x... else if y..." logic but cleaner.

        if self.hist_check.isChecked():
            target_x = self.x if self.x is not None else self.y
            if target_x:
                sns.histplot(
                    data=self.data,
                    x=target_x,
        ...
        if self.count_check.isChecked():
            # Countplot usually needs x OR y
            sns.countplot(
                data=self.data,
                x=self.x,
                hue=self.hue_variable,
        ...
        # Bivariate plots usually need both or specific orientation
        ...
        if self.swarm_check.isChecked():
            # Swarm can be x vs y or just y (categorical x)
            if self.x_option:
                sns.swarmplot(
                    data=self.data,
                    x=self.x,
