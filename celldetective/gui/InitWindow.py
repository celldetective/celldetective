...
class AppInitWindow(CelldetectiveMainWindow):
    ...
    def __init__(self, parent_window=None, software_location=None):
        ...
        self.use_gpu = False  # Default to False, check in background
    ...
    def _createActions(self):
        ...
        self.openSpreadingAssayDemo.triggered.connect(
            self.download_spreading_assay_demo
        )
        self.openCytotoxicityAssayDemo.triggered.connect(
    ...
    def replay_session(self):
        ...
        filename, _ = QFileDialog.getOpenFileName(
            self, "Select Session Log", path, "JSON Files (*.json)"
        )
        if filename:
            ...
            try:
                ...
                generic_message(
                    "Replay will start now. Please do not interact with the window.",
    ...
    def open_directory(self):
        ...
        try:
            ...
            setup_global_logging(log_file=log_file)
            logger.info("========================================")
            logger.info(f"Session started: {timestamp}")
            logger.info(f"Log file: {log_file}")
            logger.info("========================================")
        except Exception as e:
            logger.error(f"Failed to initialize session logging: {e}")
