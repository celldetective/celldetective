...
def analyze_signals(
    ...
    logger.info(
        f"The following channels will be passed to the model: {selected_signals}"
    )
        ...
        if hasattr(self, "x_val"):
            ...
            if self.show_plots:
                try:
                    ...
                    pass
            logger.info(
                "Validation set: ", classification_report(ground_truth, predictions)
            )
...
def residual_block1D(x, number_of_filters, kernel_size=7, strides=1):
    ...
    # Convolution 1
    x_conv = Conv1D(
        number_of_filters, kernel_size=kernel_size, strides=strides, padding="same"
    )(x_act)
    ...
    if strides > 1 or x.shape[-1] != number_of_filters:
        # Projection shortcut
        shortcut = Conv1D(
            number_of_filters, kernel_size=1, strides=strides, padding="same"
        )(x)
    else:
        # Identity shortcut
        shortcut = x
