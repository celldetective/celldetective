...
parser = argparse.ArgumentParser(
    description="Classify and regress the signals based on the provided model.",
...
if os.path.exists(trajectories_path):
    ...
    logger.warning("The trajectories table could not be found. Skipping position.")
    sys.exit(0)

with PositionLogger(pos):
    logger.info(f"Starting signal analysis with model: {model}, mode: {mode}")

    trajectories = analyze_signals(
        trajectories.copy(),
        model,
        interpolate_na=True,
        ...
        plot_outcome=False,
        output_dir=os.path.join(pos, "output", ""),
    )
