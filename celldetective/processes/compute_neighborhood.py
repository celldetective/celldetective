...
class NeighborhoodProcess(Process):
    ...
    def mask_contact_neighborhood(
        ...
        for d in distance:
            ...
            if mode == "two-pop":
                ...
                logger.error(
                    "Please provide a valid mode between `two-pop` and `self`..."
                )
                return None
    ...
    def distance_cut_neighborhood(
        ...
        for d in distance:
            ...
            for t in tqdm(timeline):
                ...
                logger.info(f"{self.sum_done=} {pred_time=}")
                self.queue.put([self.sum_done, pred_time])
    ...
    def run(self):
        logger.info(f"Launching the neighborhood computation...")
        if self.protocol["neighborhood_type"] == "distance_threshold":
            ...
            logger.info(f"Computation done!")
        elif self.protocol["neighborhood_type"] == "mask_contact":
            logger.info(f"Compute contact neigh!!")
